...
public interface MarketListingRepository extends JpaRepository<MarketListing, UUID> {
        ...
        /**
         * Listing ACTIVE của một item – DB đảm bảo partial unique index (chỉ 1 ACTIVE
         * per item).
        ...
        /**
         * Danh sách listing ACTIVE của một seller.
         */
        List<MarketListing> findBySellerIdAndStatusOrderByCreatedAtDesc(UUID sellerId, MarketListing.Status status);

        /**
         * Listing ACTIVE của một item thuộc seller.
         */
        Optional<MarketListing> findBySellerIdAndItemIdAndStatus(UUID sellerId, UUID itemId,
                        MarketListing.Status status);
